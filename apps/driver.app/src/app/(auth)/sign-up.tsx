...
export default function SignUpScreen() {
  ...
  const onSubmit = async () => {
    if (!name || !phone || !password) {
      Alert.alert('Missing details', 'Please fill in your name, phone number, and password.');
      return;
    }
    setLoading(true);
    try {
      await signUp(phone, password, name);
      router.push({ pathname: '/confirm', params: { phone } });
    } catch (err) {
    ...
    <ThemedView style={styles.container}>
      <SafeAreaView style={styles.safeArea}>
        ...
        <ThemedView type="backgroundElement" style={styles.form}>
          <AuthTextField label="Full name" value={name} onChangeText={setName} autoCapitalize="words" />
          <AuthTextField
            label="Phone number"
            value={phone}
        ...
        <ThemedText type="link" style={styles.footer} onPress={() => router.replace('/sign-in')}>
